...
class SectionRender extends HTMLElement {
  ...
  async updateSection(sectionIds) {
    ...
    if (Array.isArray(sectionIds)) {
      sections = sectionIds
        .map((id) =>
          Array.from(this.querySelectorAll(`[${this.renderTargetAttr}=${id}]`))
        )
        .flat();
    } else {
      sections = this.querySelectorAll(
        `[${this.renderTargetAttr}=${sectionIds}]`
      );
    }
  ...
  renderMultipleContents(sections, response, sectionIds, contentContainer) {
    ...
    const sectionsMarkup = sectionIds.reduce(
      (accumulator, sectionId, index) => {
