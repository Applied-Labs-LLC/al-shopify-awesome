class VariantChanger extends HTMLElement {
  ...
  urlReplace(value) {
    ...
    const currentUrl = this.querySelector(
      `[${currentUrlAttribute}]`
    ).getAttribute(`${currentUrlAttribute}`);
  ...
  changeInputVariantToActive(target) {
    ...
    const variantGroup = this.querySelector(
      `[${this.groupIdAttribute}="${variantId}"]`
    );

    variantGroup
      .querySelectorAll(`[${this.variantTrigger}]`)
      .forEach((variant) => {
  ...
  getSelectedOptions() {
    return Array.from(
      this.querySelectorAll(`[${this.variantActiveAttribute}]`)
    ).map((option) => option.getAttribute(this.variantValueAttribute));
  }
  ...
  async updateProductPage() {
    ...
    try {
      ...
      document.querySelector(`[${this.productSectionAttribute}]`).innerHTML =
